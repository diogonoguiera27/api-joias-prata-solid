...
class ClientesService {
    constructor(clientesRepository, validadorDadosCliente, regraRemocaoCliente) {
    ...
    async criar(data) {
        ...
        const clienteExistente = await this.clientesRepository.buscarClientePorEmail(dadosCliente.email);
        if (clienteExistente) {
    ...
    async buscarPorId(data) {
        const cliente = await this.clientesRepository.buscarClienteDetalhadoPorId(data.id);
        if (!cliente) {
    ...
    async atualizar(data) {
        ...
        const dadosCliente = this.validadorDadosCliente.validar(data);
        const clienteComMesmoEmail = await this.clientesRepository.buscarClientePorEmail(dadosCliente.email);
    ...
    async remover(data) {
        const cliente = await this.clientesRepository.buscarClienteComVinculosPorId(data.id);
        if (!cliente) {
            throw new Error("Cliente não encontrado.");
        }
...
exports.clientesService = new ClientesService(clientes_repository_1.clientesRepository, validadorDadosCliente, regraRemocaoCliente);
